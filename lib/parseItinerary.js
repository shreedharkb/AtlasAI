 ...
 * JSON embedded in text, and unescaped control characters inside JSON strings
 */
function extractJSON(text) {
  const trimmed = sanitizeControlChars(text.trim());
...
function attemptRepairTruncatedJSON(partialStr) {
  ...
  const salvagePrefix = (inputStr) => {
    let str = sanitizeControlChars(cleanCommas(inputStr.trim()));
    let inStr = false;
    let esc = false;
    const stack = [];
  ...
  try {
    ...
    // 2. If direct salvage fails, iteratively step backwards to any separator or brace/bracket up to 150 times.
    ...
    for (let attempts = 0; attempts < 150; attempts++) {
      ...
      const cutIdx = Math.max(lastComma, lastCloseBrace, lastCloseBracket, lastOpenBrace, lastOpenBracket);
